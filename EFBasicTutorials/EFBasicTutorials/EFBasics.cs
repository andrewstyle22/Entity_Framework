    ...
    class EFBasics
    {
        ...
        public static void DynamicProxyDemo()
        {
            ...
            using (var context = new SchoolDBEntities())
            {
                try
                {
                    var stud1 = context.Students.Include("Standard.Teachers").Where(s => s.StudentName == "Student1").FirstOrDefault();

                    var stud = context.Students.Include("Standard.Teachers").Where(s => s.StudentName == "Student1").FirstOrDefault();
